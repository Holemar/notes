...
二、常用SQL:
  ...
  10.创建视图:create view 视图名 as select statement;
     删除视图:DROP view 视图名;
  11.避免删除时死锁: DELETE FROM 表a a left join (SELECT 主键 FROM 表a WHERE 条件)b using(主键)
...
实例:
   ...
   10. 随机取值
    ...
    CREATE TABLE `t_data_daily` (
      ...
      `c_access_date` date NOT NULL COMMENT '日期',
      `c_name` varchar(50) NOT NULL COMMENT '数据名称',
      `c_number` bigint(20) DEFAULT '0' COMMENT '数量',
      `c_add_dt` datetime DEFAULT NULL COMMENT '添加时间',
      `c_update_dt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
      PRIMARY KEY (`c_id`),
      UNIQUE KEY `u_key` (`c_access_date`,`c_name`),
      KEY `ix_date` (`c_access_date`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='日数据表';

    -- 导出预约演示数量
    SELECT `c_access_date` AS `日期`,
     MAX(CASE `c_name` WHEN 'new_sales_lead' THEN `c_number` ELSE 0 END ) `今日预约演示`,
     MAX(CASE `c_name` WHEN 'total_sales_lead' THEN `c_number` ELSE 0 END ) `累计预约演示`
    FROM `t_data_daily` WHERE `c_name` IN ('new_sales_lead', 'total_sales_lead')
    GROUP BY `c_access_date` ORDER BY `c_access_date`;
