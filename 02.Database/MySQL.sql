...
BTree索引：
    BTree索引是MySQL数据库中最常用的索引类型。
    BTree索引的实现原理是：将索引字段的数据存放在一个平衡二叉树中，树的每个节点存放索引字段的一个值，左子树中的值小于右子树中的值。
    BTree索引的优点是通过这种方式，可以快速找到某个范围内的索引值。
...
B-TREE索引使用场景
    ...
    匹配模糊查询的前匹配，如 where act_name like '11_act%'
    匹配范围值的SQL查询，如 where act_date > '9865123547215'（not in和<>无法使用索引）
    覆盖索引的SQL查询，就是说 select 出来的字段都建立了索引


HASH 索引
    Hash 索引基于 Hash 表实现，只有查询条件精确匹配 Hash 索引中的所有列才会用到 hash 索引
    存储引擎会为 Hash 索引中的每一列都计算 hash 码，Hash 索引中存储的即 hash 码，所以每次读取都会进行两次查询
    Hash 索引无法用于排序
    使用场景: 为超长的键创建哈希索引。列值太长，导致索引体积过大，查询速度也会受到影响。
    Hash 索引不适用于区分度小的列上，如性别字段


MySQL 数据库的锁机制：
    MySQL数据库提供了两种锁机制：
    - 表级锁（Table Lock）：对整个表加锁，实现简单，开销小，加锁期间其他进程无法访问该表，并发度较低，但冲突概率低。
    - 行级锁（Row Lock）：对表中的一行加锁，并发度高，开销大，加锁期间其他进程可以访问该行，并发度高，冲突概率高。


mysql 自增主键达到最大值:
    报错: 主键重复
    解决方法:
    1. 主键改用 bigint 类型(当数据量这么大的时候，更新表结构也是个大坑)
    2. 分库分表
    3. 雪花算法
        自增可排序的 id, Long 数据类型的 64bit 大小的整数，由 1bit 的符号位、 41bit 的时间戳、 10bit 的工作机器 id 、 12bit 序列号组成。
        可以保证：所有生成的 id 按时间趋势递增，整个分布式系统内不会产重复 id。
        它有以下三个缺点:
            1. 依赖机器时钟，如果机器时钟回拨，会导致有重复id生成。
            2. 在单机上递增，但是如果在分布式环境下，每台机器时钟未必同步，可能出现不是全局递增情况。（可忽略）
            3. 64位的二进制数，化为10进制存储一般为19位，但是前端js只能保证前16位的精度，前端拿到这条数据时，会对后三位进行四舍五入的处理，丢失了精度。
