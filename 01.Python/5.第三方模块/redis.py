...
一、数据库操作
    ...
    3. 设值(添加值)
       r.set('key', 'value')   # 返回值: True
       r['key'] = 'value'  # 上句的另一种写法
       r.getset('key', 'value') # 可以在存新数据的同时,将上次存储内容同时读取出来。返回上次存储的值(没有则返回 None),并将 'value' 赋值给 'key'
    4. 取值
       r.get('key')  # 返回读取的值(没有则返回 None),如: 'bar'
       r['key']      # 类似上句,返回读取的值(没有时会报错),如: 'bar'
       r.mget('key1','key2')  # 同时取一批数据,返回值的 list
    5. 取键
       r.exists('key')  # 看是否存在这个键值,返回值: True / False
       ...
       r.randomkey()  # 随机取一个键
    6. 删除值
       r.delete('key')  # 删除成功则返回 True, 没有这键则返回 False
       r.delete('key1','key2')  # 它支持批量操作,只要有一个值删除成功则返回 True, 所有这些键都没有则返回 False
       r.flushdb()   # 删除当前数据库的所有数据,返回值: True / False

    7. 取 递增/递减的整数
       print( r.incr('key') )  # 设置一个递增的整数,每执行一次它自加1,返回自增后的整数(第一次时是1)
       print( r.decr('key2') )  # 设置一个递减的整数,每执行一次它自减1,返回自减后的整数(第一次时是-1)
    8. 查看数据类型,类型有四种： string(key,val)、list(序列)、set(集合)、zset(有序集合,多了一个顺序属性)
       print( r.type('key') ) # 返回数据的类型
    9. 其它
       r.rename('key1','key2')  # 改名,成功则返回 True, 没有这键时报错
       r.expire('key',10)  # 让数据10秒后过期(如果执行的进程终止了,此数据会马上过期,即使还没到时间也过期)
       r.ttl('key')  # 看剩余过期时间，不存在返回 None，返回剩余多少秒的整数
       r.persist('key') # 删除到期的key，有这key则返回 True，没有则返回 False
...
信息
    ...
    15.换数据库
       r.select(2)
    16.移动数据去其它数据库
       r.move('key',2)  # 将 'a' 的内容由当前数据库移动到 2 数据库

    17.序列(list)操作
       # 它是两头通的。以下操作，如果 'b' 不是 list 类型则报错
       r.lpush('key','value') # 从左边塞入,返回塞入后的 list 长度
       r.rpush('key','value') # 从右边塞入,返回塞入后的 list 长度
       r.lpop('key')  # 从左边弹出,返回弹出的值
       r.rpop('key')  # 从右边弹出,返回弹出的值
       r.rpoplpush('key', 'key2') # 从'key'的右边弹出,从左边塞入到list'key2'里面,返回弹出的值(相当于 r.lpush('key2', r.rpop('key')) )

       r.llen('key') # 返回 list 的长度
       r.lrange('key',start=0,end=-1) # 这里是列出所有,可以指定 star 和 end 列出那些值
       r.lindex('key',0) # 取出指定 index 的一个值

       # 修剪列表
       # 若 start 大于end, 则将这个list清空
       r.ltrim('key',start=0,end=3) # 只留 从0到3 的四个值,返回True

    18.集合(set)操作
       # 以下，如果 'key' 不是 set 类型则报错
       r.sadd('key', 'a')  # 添加数据,如果成功往 set 里面添加此数据则返回 True,已经有这数据则返回 False
       r.scard('key') # 判断 set 的长度,不存在为0,返回长度的整数
       r.sismember('key','a') # 判断 set 中一个对象是否存在,集合或者对象不存在都返回 False, set 中存在此值则返回 True
       r.smembers('key') # 看一个set对象。返回的是 set([交集的值列表]) 类型
       # 交集
       r.sinter('key','s2') # 求交集,返回 'key' 和 's2' 两个 set 的交集的set。
       r.sinterstore('s3','key','s2') # 求交集并将结果赋值。求得第二个参数及之后的所有 set 的交集,并将结果赋值给第一个参数的 set。返回赋值后的第一个参数的 set 的长度('s3'赋值前的值不再保存)
       # 并集
       r.sunion('key','s2') # 求并集,返回 'key' 和 's2' 两个 set 的并集的set。
       r.sunionstore('ss','key','s2','s3') # 求并集 并将结果赋值。 求得第二个参数及之后的所有 set 的并集,并将结果赋值给第一个参数的 set。返回赋值后的第一个参数的 set 的长度
       # 不同
       r.sdiff('key','s2','s3') # 求不同。在'key'中有，但在's2'和's3'中都没有的值的set
       r.sdiffstore('s4','key','s2') # 求不同 并将结果赋值。 类似 sinterstore, sunionstore, 你懂的
       r.srandmember('key') # 取个随机值。返回 set 里面的随机一个值, set 里面没有值时返回 None
       r.spop('key') # 随机取一个值，且删掉这个值。
