...
def send_mail(host, user, password, fro, to_list, subject, text, files=None, ssl=False):
    '''
    @summary: 发邮件
    ...
    try:
        # QQ邮箱必须用SSL
        if ssl or host.lower() == "smtp.qq.com":
            smtp = smtplib.SMTP_SSL(host, 465) #连接smtp服务器
...
def to_unicode(value):
    ...
    if isinstance(value, str):
        for encoding in ("utf-8", "gbk", "big5", 'unicode-escape', sys.getdefaultencoding(), "ascii"):
            try:
...
if __name__ == '__main__':
    ...
    user = "backend_program@xxxxxx.com"
    password = "xxxxxx"
    fro = u"发货系统错误汇报<backend_program@xxxxxx.com>"   #这里可以任意设置，收到信后，将按照设置显示收件人
    mail_to_list = ["fengwanli@xxxxxx.com", '292598441@qq.com'] # 收信人,发多个人需要用列表,只用字符串则只发第一个
    subject = u"发货系统有错误，请求处理"
    text = u'''这是测试内容,见到请忽略...<br/>
    现有发货不成功<font color='red'>红色字体</font>
    '''
    files = [u"E:\\WebDisk\\编程资料\\notes\\状态码.txt",
              u"E:\\WebDisk\\picture\\bb\\手机照\\IMG_20120827.jpg"
              ]
