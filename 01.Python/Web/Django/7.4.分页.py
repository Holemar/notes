...
分批处理数据：
    from apps.law.models import Law # 这里假设有这么一个 model
    level = [0, u'法律', u'行政法规', u'司法解释/文件', u'部委规章', u'地方法规', u'政党及组织文件', u'行业规范', u'地方司法解释/文件']
    paginator = Paginator(Law.objects.all().only('id', 'level_name'), 200)
    # 分批查询出来
    for page in range(1, paginator.num_pages + 1):
        object_list = paginator.page(page).object_list
        # 逐个实例处理
        for law in object_list:
            pass
