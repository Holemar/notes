...
案例3(指定 Manager 的查询条件)
    ...
    class Department(models.Model):
        ...
        company = models.ForeignKey('ucenter.Company', verbose_name='企业ID', max_length=32, default='123', db_column='c_company_id') # 外键不在同一个库，得用字符串指向
        name = models.CharField(verbose_name='部门名称', max_length=30, db_column='c_name')
        sup_deparment = models.ForeignKey('self', verbose_name='上级部门', related_name='children', null=True, db_column='c_sup_deparment_id') # 外键是自身，可以用'self'
...
判断某字段的类型
    ...
    # 参数类型判断
    model_instance = Product(...) # model_instance 是个 Model 的实例
    field = model_instance._meta.get_field('col_name')
    if isinstance(field, (models.DecimalField, models.IntegerField, models.FloatField, models.SmallIntegerField, models.BigIntegerField, models.PositiveIntegerField, models.PositiveSmallIntegerField)):
        if not is_number(v): continue # 如果前端输入错误值(非number类型)，忽略它
