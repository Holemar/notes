...
2.model操作
  ...
  # 批量新增
    Publisher.objects.bulk_create(object_list, batch_size=200) # 参数 batch_size 表示每次新增多少行

  # get_or_create
    # get_or_create 有则返回查询结果(返回: 结果类, False)， 没有则新增一个(返回: 新增后的结果类,True)
    # defaults 参数：如果查询到记录则不处理(不包含在查询条件内)。 查询不到时作为 insert 的值来用。
    # 注意： 高并发且设置了唯一键时，get_or_create 会出现 insert 导致唯一键冲突的异常
    obj, result = Publisher.objects.get_or_create(name='Apress', address='Ninth St.', website='http://www.xx.com/')
    # 语句： select * from publisher where name='Apress' and address='Ninth St.' and website='http://www.xx.com/'
    # 查询不到则会再有： insert into publisher (name,address,website) values('Apress', 'Ninth St.', 'http://www.xx.com/')
    obj, result = Publisher.objects.get_or_create(name='Apress', address='Ninth St.', defaults={'website': 'http://www.xx.com/', 'email':'daillow@gmail.com'})
    # 语句： select * from publisher where name='Apress' and address='Ninth St.'
    # 查询不到则会再有： insert into publisher (name,address,website,email) values('Apress', 'Ninth St.', 'http://www.xx.com/','daillow@gmail.com')

  # update_or_create
    # update_or_create 有则返回查询结果(返回: 结果类, False)， 没有则新增一个(返回: 新增后的结果类,True)
    # defaults 参数：如果查询到记录且对应的值跟defaults的相同则不处理(不包含在查询条件内)。 查询不到时作为 insert 的值来用。查询到但值不同则作为 update 的值来用。
    Publisher.objects.update_or_create(name='Apress', address='Ninth St.', defaults={'website': 'http://www.xx.com/', 'email':'daillow@gmail.com'})
    # 语句： select * from publisher where name='Apress' and address='Ninth St.'
    # 查询不到则会再有： insert into publisher (name,address,website,email) values('Apress', 'Ninth St.', 'http://www.xx.com/','daillow@gmail.com')
    # 查询到，但值不同则会有： update publisher set website='http://www.xx.com/', email='daillow@gmail.com' where id=xxx (id值来自于查询结果)
  ...
  # 判断是否存在/是否有这样的记录 exists
    Book.objects.filter(publisher__name="Joe").exists()

  # 节省的查询 only
    book = Book.objects.only('no', 'title', 'publisher_id', 'publisher__name').select_related('publisher').filter(confirm_status='已通过').first()
    # 这样查询返回的类，只有 only 里面的属性有值，其它属性没有值。
    # 注意：如果还需要其它属性的话，每个属性都需要单独查询一次数据库，反而性能更慢。仅在不需要其它属性时适用。
    # 外键的属性，可以用 双下划线 指定。

  # 节省的查询 defer (功能与 only 刚好相反)
    book = Book.objects.defer('no', 'title', 'publisher_id', 'publisher__name').select_related('publisher').filter(confirm_status='已通过').first()
    # 这样查询返回的类，查询语句里面会排除 defer 里面的属性，只查询其它属性的值。
    # 注意：如果还需要用 defer 属性里面的值的话，每个属性都需要单独查询一次数据库，反而性能更慢。仅在不需要 defer 里面各属性时适用。
    # 外键的属性，可以用 双下划线 指定。
  ...
  # 查询指定字段 values_list 与 values
    # values 返回一个列表，里面包含的是字典，每个字典包含字段名和值
    # values_list 也返回一个列表，但里面只包含值(如果只查一个字段，可以加上参数 flat=True 输出一维列表。没有这参数则输出的列表里面是个 tuple 类型，包含各值是 values_list 要查询的各字段)
    p_list = Publisher.objects.all().values('name').distinct() # [{'name': u'Apress'}, {'name': u'Joe'}]
    p_list = Publisher.objects.all().values_list('name').distinct() # [(u'Apress',), (u'Joe',)]
    p_list = Publisher.objects.all().values_list('name', flat=True).distinct() # [u'Apress', u'Joe']
  ...
  # 纯聚合 (注意:纯聚合 使用的是 aggregate, 而 group by 的则用 annotate)
...
3.批量插入数据
    ...
    做法3:
        # django 1.4+,  加入 bulk_create, 允许一次性插入多个类(参数 batch_size 是指一次最多插入多少个，分批插入)
