...
codePointAt(index)	|  返回字符的 UTF-16 编码。 与charCodeAt()方法类似，但该方法可以识别码点大于`0xFFFF`的字符(ES6新增)。
...
split(bystr, limit)	|  返回由bystr分割成的字符串数组(通常bystr是连接符号，如逗号或横杆)。参数limit:可选,该参数可指定返回的数组的最大长度。
...
replace(regexp,replacement)	|  替换与正则表达式匹配的第一个子串。
replaceAll(regexp,replacement)	|  在字符串中查找匹配的子串，并替换与正则表达式匹配的所有子串。(ES2021新增)
