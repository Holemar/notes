...
def get_cache_model(model_class, key, get_fun, timeout=300, cache=cache):
    """
    获取缓存的model实例
    :param model_class: model类型
    :param key: 缓存的key值
    :param get_fun: 缓存获取不到时，获取此实例的函数
    :param timeout: 超时时间，设为0则不缓存
    :param cache: 缓存类
    :return:model实例
    """
    if not timeout:
    ...
    if obj_dict is None or not isinstance(obj_dict, dict):
        ...
        if obj:
            ...
            # ForeignKey 类型的需要额外转换
            for field in obj._meta.fields:
                field_name = field.name
                if isinstance(field, (models.ForeignKey, models.UUIDField)):
                    value = obj_dict.pop(field_name, None)
                    value = uuid.UUID(value) if value and isinstance(value, basestring) and len(value) in (32, 36) else value
                    if isinstance(field, models.ForeignKey):
                        obj_dict['%s_id' % field_name] = value
                        setattr(obj, '%s_id' % field_name, value)
                    else:
                        setattr(obj, field_name, value)
                    obj_dict[field_name] = value
                #elif field_name not in obj_dict:
                #elif isinstance(field, (models.DateField, models.DateTimeField)) and (field.auto_now_add or field.auto_now):
                elif getattr(field, 'editable', False) == False:
                    obj_dict[field_name] = getattr(obj, field_name)
            cache.set(key, obj_dict, timeout=timeout)
    else:
        try:
            #obj = model_class(**obj_dict) # 当类有属性增减时，由于各子系统升级时间不同步，这行会报异常。改用下面写法
            obj = model_class()
            for k, v in obj_dict.items():
                field = obj._meta.get_field(k)
                if isinstance(field, (models.ForeignKey, models.UUIDField)):
                    v = uuid.UUID(v) if v and isinstance(v, basestring) and len(v) in (32, 36) else v
                    if isinstance(field, models.ForeignKey):
