...
class TestLogUtil(unittest.TestCase):
    ...
    def test_init_socket(self):
        ...
        log_util.init(log_file=None, level=log_util.INFO, socket=(remote_host, port1)) # 全局日志
        log_util.init(log_file=None, level=log_util.WARNING, socket=(remote_host, port1), logger_name='tasks', log_max=0) # 定义 logger_name 的
        log_util.init(log_file=None, level=log_util.WARNING, socket=(my_host, [port1, port2, port1]), logger_name='tasks.my', log_max=0) # 定义多个端口
        ...
        # 定义多个端口，且其中包含父级的
        log_util.init(log_file=None, level=log_util.INFO, socket=(remote_host, [port1, port2]), logger_name='tasks.my2', log_max=0)
        ...
        # 多次定义测试
        log_util.init(log_file=None, level=log_util.DEBUG, socket=(remote_host, [port1, port2]), logger_name='tasks.my2', log_max=0)
        my2_handlers = [h for h in my2_logger.handlers if type(h) is log_util.SocketHandler]
    ...
    # init 定义多个 syslog 测试
    def test_init_syslog(self):
        remote_host = '218.17.157.25'
        my_host = '127.0.0.1'
        port1 = 9022
        port2 = 9023

        log_util.init(log_file=None, level=log_util.INFO, syslog={'address':(remote_host, port1)}) # 全局日志
        log_util.init(log_file=None, level=log_util.WARNING, syslog={'address':(remote_host, port1)}, logger_name='tasks', log_max=0) # 定义 logger_name 的
        log_util.init(log_file=None, level=log_util.WARNING, syslog={'address':(my_host, [port1, port2, port1])}, logger_name='tasks.my', log_max=0) # 定义多个端口

        root_logger = logging.getLogger()
        root_handlers = [h for h in root_logger.handlers if type(h) is log_util.SysLogHandler]
        self.assertEqual(len(root_handlers), 1) # 定义了1个全局的 syslog
        self.assertEqual(root_handlers[0].address[0], remote_host)
        self.assertEqual(root_handlers[0].address[1], port1)
        self.assertEqual(root_handlers[0].level, log_util.INFO)

        tasks_logger = log_util.getLogger('tasks')
        tasks_handlers = [h for h in tasks_logger.handlers if type(h) is log_util.SysLogHandler]
        self.assertEqual(len(tasks_handlers), 0) # 由于已经定义了全局的，所以子级的相同 syslog 不用重复发

        my_logger = log_util.getLogger('tasks.my')
        my_handlers = [h for h in my_logger.handlers if type(h) is log_util.SysLogHandler]
        self.assertEqual(len(my_handlers), 2) # 定义了2个 syslog
        self.assertEqual(my_handlers[0].address[0], my_host)
        self.assertEqual(my_handlers[1].address[0], my_host)
        self.assertEqual(my_handlers[0].level, log_util.WARNING)
        self.assertEqual(my_handlers[1].level, log_util.WARNING)
        self.assertEqual(set([my_handlers[0].address[1], my_handlers[1].address[1]]), set([port1, port2]))

        # 定义多个端口，且其中包含父级的
        log_util.init(log_file=None, level=log_util.INFO, syslog={'address':(remote_host, [port1, port2])}, logger_name='tasks.my2', log_max=0)
        my2_logger = logging.getLogger('tasks.my2')
        my2_handlers = [h for h in my2_logger.handlers if type(h) is log_util.SysLogHandler]
        self.assertEqual(len(my2_handlers), 1) # 定义了1个 socket， 另一个端口由于父级已经包含，不再重复发
        self.assertEqual(my2_handlers[0].address[0], remote_host)
        self.assertEqual(my2_handlers[0].address[1], port2)
        self.assertEqual(my2_handlers[0].level, log_util.INFO)

        # 多次定义测试
        log_util.init(log_file=None, level=log_util.DEBUG, syslog={'address':(remote_host, [port1, port2])}, logger_name='tasks.my2', log_max=0)
        my2_handlers = [h for h in my2_logger.handlers if type(h) is log_util.SysLogHandler]
        self.assertEqual(len(my2_handlers), 2) # 定义了2个 socket， 父级的级别高，这里需要继续发
        self.assertEqual(my2_handlers[0].address[0], remote_host)
        self.assertEqual(my2_handlers[0].level, log_util.DEBUG)
        self.assertEqual(my2_handlers[1].address[0], remote_host)
        self.assertEqual(my2_handlers[0].level, log_util.DEBUG)
        self.assertEqual(set([my2_handlers[0].address[1], my2_handlers[1].address[1]]), set([port1, port2]))
