...
def add_syslog_handler(level, logger, address=('localhost', logging.handlers.SYSLOG_UDP_PORT), facility=SysLogHandler.LOG_USER, socktype=None):
    """
    给 logger 加上 syslog 日志处理
    :param {bool | string} level: 日志级别,默认级别:INFO
    :param {logging.Logger} logger: 指定日志实例(SysLogHandler 会加入到这个 logger 里面)
    :param {tuple<string,int | list<int>>} address: 把日志发往远程SysLog上的(ip/host,端口号)配置(为空则不发)
    :param {int} facility: syslog日志约定级别
    :param {int} socktype: syslog的sock类型约定
    """
    if not address: return
    syslog_host, syslog_ports = address
    if not syslog_host or not syslog_ports: return

    syslog_ports = [syslog_ports] if isinstance(syslog_ports, (int, long)) else syslog_ports
    for socket_port in syslog_ports:
        # 先移除旧的同socket处理器，省去修改旧处理器的麻烦
        logger.handlers[:] = [h for h in logger.handlers if not (type(h) is SysLogHandler and h.address == (syslog_host, socket_port))]
        # 如果父级已经发了同一份socket日志，则没必要再发同样内容(避免同一个日志重复两次)
        parent_name = logger.name
        parent_handlers = []
        while parent_name.rfind('.') > 0:
            parent_name = parent_name[:parent_name.rfind('.')]
            parent_logger = getLogger(parent_name, add_parent_filter=False)
            parent_handlers.extend([h for h in parent_logger.handlers if type(h) is SysLogHandler and h.address == (syslog_host, socket_port) and h.level <= level])
            if parent_handlers: break
        else:
            parent_handlers.extend([h for h in logging.root.handlers if type(h) is SysLogHandler and h.address == (syslog_host, socket_port) and h.level <= level])
        if not parent_handlers:
            # 如果父级没有重复日志，则添加本级日志处理器
            handler = SysLogHandler(address=(syslog_host, socket_port), facility=facility, socktype=socktype)
            handler.setLevel(level)
            logger.addHandler(handler)
